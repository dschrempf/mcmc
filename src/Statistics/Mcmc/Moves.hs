...
differentiate between the proposal distribution and how it affects the state.
...
'mvSlide' would be @move (normalDistribution mean variance) (+)@. This
specification is more involved, and may be too complicated. Especially, we need
to know the probability of jumping back, and so we need to know the reverse of
the operator. However, it also allows specification of new moves with great
ease.
...
import Statistics.Mcmc.Moves.Normal
